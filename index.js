...
const init = async () => {
    ...
    server.route({
        ...
        handler: async (request, h) => {
            ...
            // android won't even show Hello
            // return 'Hello';

            if (data.file) {
                const name = data.file.hapi.filename;
                const path = __dirname + "/uploads/" + name;
                const encodedPath = __dirname + "/uploads/encoded_" + name; // we will require this encodedPath later
                    ...
                    data.file.on('end', async (err) => { 
                        ...
                        ffmpeg()
                            ...
                            .on('end', async () => {
                                ...
                                const savedFile = await fs.readFileSync(encodedPath);
                                if (!savedFile) {
